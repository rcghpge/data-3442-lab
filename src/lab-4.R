...
# ============================================================
# Mean Plot for PLANTS1 Data (Mean and Nitrogen Percentages)
# ============================================================
...
plot1 <- ggplot(means_plants1, aes(x = as.numeric(as.character(water)), y = mean_pctnit, color = species)) +
  ...
  theme_minimal()
ggsave("plants1-mean.pdf", plot = plot1, width = 8, height = 6)
...
pdf("nitrogen-plants1-plots.pdf")
par(mfrow = c(2, 2))
plot(anova_plants1)
dev.off()
...
# ============================================================
# Mean Plot for PLANTS2 Data (Fresh Biomass)
# ============================================================

means_plants2 <- plants2 %>%
  group_by(species, water) %>%
  summarise(mean_fbiomass = mean(fbiomass, na.rm = TRUE), .groups = 'drop')

plot2 <- ggplot(means_plants2, aes(x = as.numeric(as.character(water)), y = mean_fbiomass, color = species)) +
  geom_line() + geom_point() +
  labs(title = "Mean Fresh Biomass by Water Level and Species",
       x = "Water Level (mm)",
       y = "Mean Fresh Biomass") +
  theme_minimal()
...
# Print to Console
cat("=== Lab 4 - Plant Species Study ANOVA and Regression Results ===\n")
cat("\n=== PLANTS1 Two-Way ANOVA ===\n")
print(anova_summary_plants1)
